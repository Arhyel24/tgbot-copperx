...
import { CheckWalletBalance, Transaction, Wallet } from "@/utils/types.js";
...
const handleViewWallets = async (
  ...
  try {
    ...
    wallets.forEach((wallet: Wallet, index: string) => {
      message +=
        `🔹 *Wallet ${index + 1}*\n` +
...
const handleCheckBalances = async (
  ...
  try {
    ...
    data.forEach((wallet: CheckWalletBalance) => {
...
const handleSetDefaultWallet = async (
  ...
  try {
    ...
    let buttons = data.map((wallet: Wallet) => [
      {
...
const handleTransactionHistory = async (
  ...
  try {
    ...
    data.data.forEach((txn: Transaction, index: string) => {
      const amount =
