...
const handleCheckKYC = async (
  ...
  try {
    ...
    const isApproved = kycData.data.every(
      (kyc: KYCData) => kyc.status.toUpperCase() === "APPROVED"
    );
...
const handleViewBankAccounts = async (
  chatId: number,
  bot: TelegramBot,
  jwtToken: string
) => {
  ...
  try {
    ...
    const bankAccounts = accounts.filter(
      (account: Account) => account.type === "bank_account"
    );
    ...
    bankAccounts.forEach((account: Account, index: number) => {
