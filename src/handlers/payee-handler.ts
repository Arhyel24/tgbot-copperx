import { Payee } from "@/utils/types.js";
...
const getPayees = async (
  ...
  try {
    ...
    data.data.forEach((payee: Payee, index: number) => {
      message += `*${index + 1}. ${
    ...
    await bot.sendMessage(chatId, `❌ *${(error as Error).message}*`, {
      parse_mode: "Markdown",
      reply_markup: {
        inline_keyboard: [
          [{ text: "⬅️ Back to Payees", callback_data: "payees" }],
...
const addPayee = async (chatId: number, bot: TelegramBot, jwtToken: string) => {
  try {
...
const editPayees = async (
  ...
  try {
    ...
    const buttons = data.data.map((payee: Payee) => [
      {
...
const listPayeesForDeletion = async (
  chatId: number,
  bot: TelegramBot,
  jwtToken: string
) => {
  try {
    ...
    const payeeButtons = data.data.map((payee: Payee) => [
      {
