...
const base_url: string = "https://income-api.copperx.io/api/transfers";
...
const minAmount: bigint = BigInt(100_000_000);
const maxAmount: bigint = BigInt(5_000_000_000_000);

const emailTransfer = async (
  chatId: number,
  bot: TelegramBot,
  accessToken: string,
  userId?: string
...
const walletTransfer = async (
  ...
  try {
    let transferData: TransferData = { currency: "USDC" };
...
const bulkTransfer = async (
  ...
  try {
    ...
    let bulkData: { requests: BulkTransferRequest[] } = { requests: [] };
    ...
    for (let i = 0; i < count; i++) {
      ...
      let walletAddress: string = await promptUser(
        bot,
        chatId,
      ...
      let amount: string = await captureValidAmount(
        bot,
        chatId,
        minAmount,
        maxAmount
      ...
      let purpose: string = await captureValidPurpose(
        bot,
        chatId,
        purposeOptions
      );
...
const promptUser = (
  ...
  return new Promise((resolve) => {
    ...
    const listener = (msg: { chat: { id: number }; text: string }) => {
...
const captureCustomerData = async (
  bot: any,
  chatId: number
...
const captureValidAmount = async (
  ...
  while (true) {
    let amountInput = await promptUser(
      bot,
      chatId,
      "💰 Enter transfer amount: Min: 100, Max: 5,000,000:\n_(Type 'cancel' to return to the transfer menu)_"
    );

    if (amountInput.toLowerCase() === "cancel") return "cancel";
...
const captureValidPurpose = async (
  bot: TelegramBot,
  chatId: number,
...
const cancelTransfer = (chatId: number, bot: TelegramBot) => {
  bot.sendMessage(
    chatId,
