...
const handleViewWallets = async (chatId, bot, jwtToken) => {
    ...
    try {
        ...
        if (!wallets || wallets.length === 0) {
            return bot.sendMessage(chatId, "🚫 No wallets found. Please create one.", {
                reply_markup: {
                    inline_keyboard: [
    ...
    try {
        ...
        data.forEach((wallet) => {
            let balanceInfo = wallet.balances
                ...
                .join("\n");
            message += `📍 *Network:* ${getNetworkName(wallet.network)}\n${balanceInfo}\n\n`;
        });
        bot.sendMessage(chatId, message, {
            parse_mode: "Markdown",
            reply_markup: {
...
const handleSetDefaultWallet = async (chatId, bot, jwtToken) => {
    ...
    try {
        ...
        const data = await response.json();
        if (!data || data.length === 0) {
            return bot.sendMessage(chatId, "🚫 No wallets available. Please create one first.", {
                reply_markup: {
                    inline_keyboard: [
                        [{ text: "➕ Create Wallet", callback_data: "create_wallet" }],
                        [{ text: "🔙 Back to Wallets", callback_data: "wallet" }],
        ...
        let buttons = data.map((wallet) => [
            {
                text: `${wallet.isDefault ? "✅ " : "⭐ "} ${getNetworkName(wallet.network)}`,
...
const handleTransactionHistory = async (chatId, bot, jwtToken) => {
    bot.sendMessage(chatId, "⏳ Retrieving your latest transactions... Please wait.");
    try {
...
export { handleCheckBalances, handleSetDefaultWallet, handleTransactionHistory, handleViewWallets, getNetworkName, };
