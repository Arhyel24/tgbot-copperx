const handleCheckKYC = async (chatId, bot, jwtToken) => {
    try {
        ...
        if (!kycData.data.length) {
            return bot.sendMessage(chatId, "🚫 No KYC record found.\n\n" +
        ...
        const kycList = kycData.data
            ...
            return (`🛂 <b>KYC Status:</b>\n\n` +
                `📍 <b>Status:</b> ${status}\n` +
                `👥 <b>Type:</b> ${kyc.type}\n` +
                `🌍 <b>Country:</b> ${kyc.country.toUpperCase()}\n\n`);
        })
            .join("\n");
        const isApproved = kycData.data.every((kyc) => kyc.status.toUpperCase() === "APPROVED");
        let message = kycList;
        if (!isApproved) {
            message +=
...
const handleViewProfile = async (chatId, bot, jwtToken) => {
    try {
        ...
        let message = `👤 *Your Profile Details:*\n\n` +
            `📧 *Email:* ${user.email}\n` +
...
const showUserPoints = async (chatId, bot, jwtToken) => {
    try {
        const response = await fetch("https://income-api.copperx.io/api/points/total", {
            method: "GET",
...
const handleViewBankAccounts = async (chatId, bot, jwtToken) => {
    ...
    try {
        ...
        const bankAccounts = accounts.filter((account) => account.type === "bank_account");
        if (bankAccounts.length === 0) {
            return bot.sendMessage(chatId, "🚫 No bank accounts found. You can add a new one below.", {
                reply_markup: {
                    inline_keyboard: [
                        [
                            {
        ...
        bankAccounts.forEach((account, index) => {
            ...
            message += `⭐ *Default Account:* ${account.isDefault ? "Yes" : "No"}\n\n`;
        });
        bot.sendMessage(chatId, message, {
            parse_mode: "Markdown",
            reply_markup: {
        ...
        bot.sendMessage(chatId, `❌ *Error fetching bank accounts.*\n\n⚠️ ${error.message || "Please try again later."}`, {
            parse_mode: "Markdown",
            reply_markup: {
                inline_keyboard: [
                    [{ text: "🔙 Back to Accounts", callback_data: "account" }],
...
export { handleCheckKYC, handleViewProfile, showUserPoints, handleViewBankAccounts, };
