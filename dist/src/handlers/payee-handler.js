...
const getPayees = async (chatId, bot, jwtToken) => {
    try {
        ...
        data.data.forEach((payee, index) => {
            message += `*${index + 1}. ${payee.displayName || payee.nickName || "Unnamed"}*\n`;
            message += `📧 *Email:* ${payee.email}\n`;
            if (payee.phoneNumber)
                message += `📞 *Phone:* ${payee.phoneNumber}\n`;
            if (payee.hasBankAccount) {
                message += `🏦 *Bank:* ${payee.bankAccount.bankName || "N/A"} (${payee.bankAccount.country.toUpperCase()})\n`;
                message += `💳 *Account Type:* ${payee.bankAccount.bankAccountType}\n`;
            }
            message += `🕒 *Added On:* ${new Date(payee.createdAt).toLocaleDateString()}\n\n`;
        });
...
const addPayee = async (chatId, bot, jwtToken) => {
    try {
        const email = await promptUser(bot, chatId, "📧 *Enter the payee's email address:*");
        if (!/\S+@\S+\.\S+/.test(email)) {
            return bot.sendMessage(chatId, "❌ *Invalid email format. Try again.*", {
                parse_mode: "Markdown",
            });
        }
        const nickName = await promptUser(bot, chatId, "🏷️ *Enter a nickname for this payee:*");
        if (!nickName) {
        ...
        await bot.sendMessage(chatId, `✅ *Payee added successfully!*\n\n📧 Email: ${email}\n🏷️ Nickname: ${nickName}`, {
            parse_mode: "Markdown",
            reply_markup: {
                inline_keyboard: [
                    [{ text: "⬅️ Back to Payees", callback_data: "payees" }],
...
const editPayee = async (chatId, bot, jwtToken, payeeId) => {
    try {
        ...
        const phoneNumber = await promptUser(bot, chatId, "📞 *Enter phone number:*");
        const updatedPayee = { nickName, firstName, lastName, phoneNumber };
        const response = await fetch(`https://income-api.copperx.io/api/payees/${payeeId}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${jwtToken}`,
        ...
        await bot.sendMessage(chatId, `✅ *Payee updated successfully!*\n\n📛 *Nickname:* ${nickName}\n👤 *Name:* ${firstName} ${lastName}\n📞 *Phone:* ${phoneNumber}`, {
            parse_mode: "Markdown",
            reply_markup: {
                inline_keyboard: [
                    [{ text: "⬅️ Back to Payees", callback_data: "payees" }],
...
const confirmDeletePayee = async (chatId, bot, payeeId) => {
    await bot.sendMessage(chatId, "⚠️ *Are you sure you want to delete this payee?*", {
        parse_mode: "Markdown",
        reply_markup: {
            inline_keyboard: [
                [
...
export { getPayees, addPayee, editPayees, editPayee, listPayeesForDeletion, confirmDeletePayee, deletePayee, };
